...
public class GeneratePlatform : MonoBehaviour
{
    ...
    void InitialiseGrid()
    {
        ...
        groundPlatSize = groundPlat.GetComponent<Collider>().bounds.size;
        ...
        if (diffx > diffz) //X
        {
            ...
            startPos = new Vector3(startObjPos.x + startObjSize.x / 2 + groundPlatSize.x , startObjPos.y, startObjPos.z);
            endPos = new Vector3(endObjPos.x - endObjSize.x, endObjPos.y, endObjPos.z);

            //Set parameters for the grid
            mapLength = Mathf.FloorToInt(Mathf.Abs(startPos.x - endPos.x) / groundPlatSize.x);
            mapWidth = 2;

            //Define the area
            areaSize = new Vector3(diffx, startPos.y - (startObjSize.y - startObjSize.y / 2), groundPlatSize.z * mapWidth);
            ...
            for (int x = 0; x < mapLength; x++)
            {
                ...
                {
                    //Debug.Log(x + ",0 is a 1");
                }
                else if (map[x,1] == 1)
                {
                    //Debug.Log(x + ",1 is a 1");
                }
            }
            ...
            for (int x = 0; x < mapLength; x++)
            {
                for (int z = 0; z < mapWidth; z++)
                {
                    pos[x, z] = new Vector3(startPos.x + (groundPlatSize.x + 2) * x, startPos.y - (startObjSize.y - startObjSize.y / 2), startPos.z + (groundPlatSize.z + 5) * z - groundPlatSize.z / 2);
                }
            }
            ...
            for (int x = mapLength - 1; x >= 0; x--)
            {
                for (int z = 0; z < mapWidth; z++)
                {
                    if (map[x, z] == 1)
                    {
                        objMap[x, z] = groundPlat;
                        Instantiate(objMap[x, z], pos[x, z], transform.rotation);                        
                    }
                    else if (map[x,z] == 2)
                    {
                        objMap[x, z] = timedPlat;
                        Instantiate(objMap[x, z], pos[x, z], transform.rotation);
                        
                    }
                    else if (map[x,z] == 3)
                    {
                        objMap[x, z] = horizontalPlat;
                        Instantiate(objMap[x, z], pos[x, z], transform.rotation);                        
                        for (int i = x+1; i < mapLength; i++)
                        {
                            for (int j = 0; j < mapWidth; j++)
                            {
                                if (objMap[i,j] == groundPlat)
                                {
                                    objMap[x, z].GetComponent<HorizontalPlatform>().PCG_target_pos = objMap[i, j].transform.position;
                                    Debug.Log("x,z:" + x + "," + z);
                                    Debug.Log("objMap[i, j]: " + objMap[i, j]);
                                }
                                else
                                {
                                    objMap[x, z].GetComponent<HorizontalPlatform>().PCG_target_pos = endObject.transform.position;
                                }
        ...
        else //Z
        {
            ...
            //Distance between boundary checkpoints and PCG egde platforms
            startPos = new Vector3(startObjPos.x, startObjPos.y, startObjPos.z + startObjSize.z / 2 + groundPlatSize.z);
            endPos = new Vector3(endObjPos.x, endObjPos.y, endObjPos.z - endObjSize.z);
            ...
            //Set parameters for the grid
            mapLength = Mathf.FloorToInt(Mathf.Abs(startPos.z - endPos.z) / groundPlatSize.z);
            mapWidth = 2;

            //Define the area
            areaSize = new Vector3(groundPlatSize.x * mapWidth, startPos.y - (startObjSize.y - startObjSize.y / 2), diffz);
            //Debug.Log(mapLength);
            //Debug.Log(mapWidth);
            ...
            for (int x = 0; x < mapLength; x++)
            {
                ...
                {
                    //Debug.Log(map[x, z]);
                    Vector3 pos = new Vector3(startPos.x + (groundPlatSize.x + 5) * z - groundPlatSize.x, startPos.y - (startObjSize.y - startObjSize.y / 2), startPos.z + (groundPlatSize.z + 2) * x);
                    if (map[x, z] == 1)
                    {
                        Instantiate(groundPlat, pos, transform.rotation);
                        objMap[x, z] = groundPlat;
                    }
                    else if (map[x,z] == 2)
                    {
                        Instantiate(timedPlat, pos, transform.rotation);
                        objMap[x, z] = timedPlat;
                    }
                    else if (map[x,z] == 3)
                    {
                        Instantiate(horizontalPlat, pos, transform.rotation);
                        objMap[x, z] = horizontalPlat;
                        //horizontalScript = //TODO
                    }
                }
            }
        }
